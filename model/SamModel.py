...
class SoftModel():
	...
	def __init__(self, input_feature, input_captions, voc_size, d_w2v, output_dim, 
		...
		## edited by zjc
		print('SamModel.py: __init__: self.enc_in_shape:', self.enc_in_shape, 'self.decoder_input_shape:', self.decoder_input_shape)
	...
	def init_embedding_matrix(self):
		...
		'''
		print('SamModel.py: init_embedding_matrix starts ...')
		voc_size = self.voc_size
		d_w2v = self.d_w2v	
		print('SamModel.py: init_embedding_matrix: voc_size=', voc_size, 'd_w2v=', d_w2v)

		np_mask = np.vstack((np.zeros(d_w2v),np.ones((voc_size-1,d_w2v))))
		T_mask = tf.constant(np_mask, tf.float32, name='LUT_mask')
		print('SamModel.py: init_embedding_matrix: T_mask:',T_mask.get_shape().as_list())
		...
		print('SamModel.py: init_embedding_matrix: T_w2v:',T_w2v.get_shape().as_list())
		print('SamModel.py: init_embedding_matrix ends ...')
		return T_w2v, T_mask 
	...
	def beamSearchDecoder(self, initial_state, input_feature, b_input_feature):
		...
		def take_step_zero(x_0, h_0):
			## zjc
			print('SamModel.py: beamSearchDecoder: take_step_zero starts ...')
			x_0 = tf.gather(self.T_w2v,x_0)*tf.gather(self.T_mask,x_0)
			print('SamModel.py: beamSearchDecoder: take_step_zero: x_0:', x_0.get_shape().as_list())
			x_0 = tf.reshape(x_0,[self.batch_size*self.beam_size,self.d_w2v])
			print('SamModel.py: beamSearchDecoder: take_step_zero: x_0:', x_0.get_shape().as_list())
			
			h = step(x_0,h_0)
			print('SamModel.py: beamSearchDecoder: take_step_zero: h:', h.get_shape().as_list())
			...
			print('SamModel.py: beamSearchDecoder: take_step_zero: logprobs_batched:', logprobs_batched.get_shape().as_list())
			
			print('SamModel.py: beamSearchDecoder: take_step_zero: logprobs_batched[:,0,:]:', logprobs_batched[:,0,:].get_shape().as_list())
			past_logprobs, indices = tf.nn.top_k(
			        logprobs_batched[:,0,:],self.beam_size)
			print('SamModel.py: beamSearchDecoder: take_step_zero: past_logprobs:', past_logprobs.get_shape().as_list(),
		...
		def test_step(time, x_t, h_tm1, past_symbols, past_logprobs, finished_beams, logprobs_finished_beams):
			...
			print('SamModel.py: beamSearchDecoder: test_step: logprobs:', logprobs.get_shape().as_list(), 
			...
			print('SamModel.py: beamSearchDecoder: test_step: past_logprobs:', past_logprobs.get_shape().as_list(),
			...
			print('SamModel.py: beamSearchDecoder: test_step: beam_past_symbols:', beam_past_symbols.get_shape().as_list())
			...
			print('SamModel.py: beamSearchDecoder: test_step: past_symbols:', past_symbols.get_shape().as_list())
			...
			print('SamModel.py: beamSearchDecoder: test_step: cond1:', cond1.get_shape().as_list(), cond1)

			for_finished_logprobs = tf.where(cond1,past_logprobs,tf.ones_like(past_logprobs,tf.float32)* -1e5)
			print('SamModel.py: beamSearchDecoder: test_step: for_finished_logprobs:', for_finished_logprobs)
			...
			print('SamModel.py: beamSearchDecoder: test_step: done_indice_max:', done_indice_max, 'logprobs_done_max:', logprobs_done_max)
			...
			print('SamModel.py: beamSearchDecoder: test_step: cond2:',cond2, 'cond3:',cond3, 'cond4:',cond4)
			...
			print('SamModel.py: beamSearchDecoder: test_step: symbols:',symbols, 'h:',h)
			print('SamModel.py: beamSearchDecoder: test_step: past_symbols:',past_symbols, 'past_logprobs:',past_logprobs)
			print('SamModel.py: beamSearchDecoder: test_step: finished_beams:',finished_beams, 
		...
		captions = self.input_captions
		## zjc
		print('SamModel.py: beamSearchDecoder: captions:', captions.get_shape().as_list())
		...
		print('SamModel.py: beamSearchDecoder: finished_beams:', finished_beams.get_shape().as_list(), 
		...
		print('SamModel.py: beamSearchDecoder: after tile, x_0',x_0.get_shape().as_list())

		h_0 = tf.expand_dims(initial_state,dim=1)
		print('SamModel.py: beamSearchDecoder: h_0',h_0.get_shape().as_list())
		h_0 = tf.reshape(tf.tile(h_0,[1,self.beam_size,1]),[self.batch_size*self.beam_size,self.output_dim])
		print('SamModel.py: beamSearchDecoder: after tile, h_0',h_0.get_shape().as_list())
		...
		print('SamModel.py: beamSearchDecoder: after take_step_zero, symbols:',symbols,
	...
	def build_model(self):
		...
		print('SamModel.py: build_model: after encoder, last_output:', last_output.get_shape().as_list(),
		...
		## edited by zjc 2018/08/22
		print('SamModel.py: build_model: after decoder, predict_score:', predict_score.get_shape().as_list(),
			'predict_words:', predict_words.get_shape().as_list(), 'loss_mask:', loss_mask.get_shape().as_list())
		print('SamModel.py: build_model: build beamSearchDecoder ...')
		finished_beam, logprobs_finished_beams, past_symbols = self.beamSearchDecoder(last_output, f_encoder_output, b_encoder_output)
		print('SamModel.py: build_model: after beamSearchDecoder, finished_beam:', finished_beam.get_shape().as_list(),
...
class HardModel():
	...
	def init_parameters(self):
		...
		if self.output_dim!=self.enc_in_shape[-1]:
                        print "$$$init$$$	output_dim:",self.output_dim,' env_in_shape:',self.enc_in_shape[-1]#new 
			print('the dimension of input feature != hidden size')
			self.liner_W = tf.get_variable("liner_W",[self.enc_in_shape[-1], self.output_dim],
				initializer=tf.random_normal_initializer(stddev=1. / math.sqrt(self.enc_in_shape[-1])))
